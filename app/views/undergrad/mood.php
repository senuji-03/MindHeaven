...
══════════════════════════════════════════════ */
  ...
  .btn--primary {
    background: var(--primary);
    color: #fff;
    box-shadow: 0 2px 8px rgba(61, 139, 110, .20);
  }

  .btn--primary:hover {
    background: var(--primary-dark);
    transform: translateY(-1px);
    box-shadow: 0 5px 16px rgba(61, 139, 110, .30);
  }
  ...
  <div class="mp-header">
    <div class="mp-header__inner">
      ...
      <div style="display: flex; gap: 10px;">
        <button class="btn-log" style="background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.3);" onclick="downloadWellnessPDF()">
  ...
  <div class="mp-body">
    ...
    <div class="smart-grid">
      ...
      <div class="smart-card">
        <div class="smart-card__hd">
          <i class="fas fa-lightbulb" style="color: #FBBF24;"></i>
          <h4 class="smart-card__title">Wellness Insights</h4>
        </div>
        <div class="smart-card__bd" id="insightsBody">
          <p class="insight-text">Loading insights based on your data...</p>
        </div>
      </div>
      ...
      <div class="smart-card">
        <div class="smart-card__hd">
          ...
          <h4 class="smart-card__title">Smart Recommendations</h4>
        </div>
        <div class="smart-card__bd">
          <div class="rec-list" id="recommendationsList">
            <!-- Dynamic recs will go here -->
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="log-detail" id="logDetail">
          <div class="fg-grid">
            ...
            <div class="fg fg-full">
              ...
              <div class="sl-row">
                <input type="range" id="moodIntensity" min="1" max="10" value="5" oninput="updateSlider(this)">
            ...
            <!-- Notes -->
            <div class="fg fg-full">
              <label class="flabel"><i class="fas fa-note-sticky"></i> Notes <span
                  style="font-weight:400;color:var(--text-secondary)">(optional)</span></label>
              <textarea class="finput" id="moodNotes" rows="2"
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('#moodPicker .m-pill').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        document.getElementById('selectedMood').value = moodVal;
        document.getElementById('logDetail').classList.add('vis');
        // Update analytics dynamically based on current selection
        updateAnalytics(moodVal);
      });
    });
    ...
    // Auto-open if redirected with ?log=1
  ...
  async function loadRecords() {
    try {
      ...
      renderCalendar();
      renderChart();
      updateAnalytics(); // Trigger Smart Analytics
      updateSlider(document.getElementById('moodIntensity'));
    } catch (e) {
  ...
  function renderChart() {
    ...
    // Set actual canvas resolution to match its visible size for sharp rendering
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      data.push(recs.length
        ? parseFloat((recs.reduce((s, r) => s + (parseInt(r.mood_level) || 5), 0) / recs.length).toFixed(1))
        : null);
    }
    ...
    const padding = 20;
    const bottomPadding = 30; // space for x labels
    
    ctx.clearRect(0, 0, w, h);

    // Filter out nulls to find max/min or default to 0-10
    const maxY = 10;
    const minY = 0;
    const rangeY = maxY - minY;

    // Draw horizontal grid lines
    ctx.strokeStyle = 'rgba(214,228,221,.5)';
    ctx.lineWidth = 1;
    ctx.fillStyle = '#6B8C7E';
    ctx.font = '10px Arial';
    ctx.textAlign = 'right';
    ...
    for (let i = 0; i <= gridSteps; i++) {
        ...
        const y = h - bottomPadding - ((val - minY) / rangeY) * (h - padding - bottomPadding);
    ...
    // Map data points
    const points = [];
    const stepX = (w - padding - 30) / (days - 1);
    
    data.forEach((val, i) => {
        if (val !== null) {
            const x = padding + 25 + (i * stepX);
            const y = h - bottomPadding - ((val - minY) / rangeY) * (h - padding - bottomPadding);
    ...
    if (points.length > 0) {
        ctx.beginPath();
        ctx.moveTo(points[0].x, h - bottomPadding);
        points.forEach(p => ctx.lineTo(p.x, p.y));
        ctx.lineTo(points[points.length - 1].x, h - bottomPadding);
        ctx.closePath();
        
        const gradient = ctx.createLinearGradient(0, padding, 0, h - bottomPadding);
        gradient.addColorStop(0, 'rgba(61,139,110,.2)');
        gradient.addColorStop(1, 'rgba(61,139,110,.0)');
    ...
    // Draw X labels (only some to avoid crowding)
    ctx.fillStyle = '#6B8C7E';
    ctx.font = '9px Arial';
    ...
    data.forEach((_, i) => {
        if (i % 3 === 0 || i === days - 1) { // Show roughly every 3rd label
            const x = padding + 25 + (i * stepX);
            ctx.fillText(labels[i], x, h - bottomPadding + 8);
        }
    });
  }
  ...
  // ─── Analytics Engine ──────────────────────────────────────────────
  function updateAnalytics(selectedMood = null) {
    if (!records.length && !selectedMood) {
      document.getElementById('insightsBody').innerHTML = '<p class="insight-text">Not enough data yet. Log some moods to see insights!</p>';
      document.getElementById('recommendationsList').innerHTML = '<p class="insight-text">Share how you feel to get personalized tips.</p>';
      return;
    }

    const latestMood = selectedMood || (records.length ? records[0].mood_type : null);
    const pastWeek = records.filter(r => {
      const d = new Date(r.recorded_at);
      const now = new Date();
      return (now - d) < (7 * 24 * 60 * 60 * 1000);
    });

    renderInsights(latestMood, pastWeek, !!selectedMood);
    renderRecommendations(latestMood);
  }

  function renderInsights(mood, pastWeek, isTemporary = false) {
    const container = document.getElementById('insightsBody');
    let insights = [];

    // Real-time Selected Mood Insight
    if (mood) {
      const m = mood.toLowerCase();
      if (['stressed', 'anxious', 'angry'].includes(m)) {
        insights.push(`Feeling <span class="insight-highlight">${m}</span>? It's important to prioritize grounding right now. See our recommendations for immediate support.`);
      } else if (['tired', 'sad'].includes(m)) {
        insights.push(`Logging <span class="insight-highlight">${m}</span>. This is a good time to focus on low-energy self-care habits and rest.`);
      } else if (['happy', 'grateful', 'excited'].includes(m)) {
        insights.push(`Wonderful to see you're feeling <span class="insight-highlight">${m}</span>! This positive momentum is a great time to tackle your more challenging goals.`);
      } else {
        insights.push(`You've selected <span class="insight-highlight">${m}</span>. Let's see how this fits into your overall wellness pattern this week.`);
      }
    }

    // Trend Analysis (only if enough data)
    if (pastWeek.length > 3) {
      const avg = pastWeek.reduce((s, r) => s + parseInt(r.mood_level), 0) / pastWeek.length;
      if (avg >= 7) insights.push('Overall <span class="insight-highlight">emotional resilience</span> has been high this week. You\'re doing great!');
      else if (avg <= 4 && !isTemporary) insights.push('It\'s been a <span class="insight-highlight">tough week</span> globally. Consider booking a quick check-in with a counselor.');
    }
    // Sleep/Exercise Correlations (Historical)
    if (!isTemporary) {
      const sleepIssues = pastWeek.filter(r => r.sleep_hours && parseFloat(r.sleep_hours) < 6 && r.mood_level < 5);
      if (sleepIssues.length >= 2) {
        insights.push('Pattern: Your mood often dips when you get <span class="insight-highlight">less than 6h sleep</span>.');
      }
      const exerciseBoost = pastWeek.filter(r => r.exercise_minutes && parseInt(r.exercise_minutes) > 20 && r.mood_level > 6);
      if (exerciseBoost.length >= 1) {
        insights.push('<span class="insight-highlight">Exercise boost</span>: Physical activity seems to directly lift your mood intensity!');
      }
    }
    ...
    const counts = Object.entries(triggerCounts).sort((a,b) => b[1] - a[1]);
    if (counts.length > 0) {
      insights.push(`The trigger <span class="insight-highlight">"${counts[0][0]}"</span> has been most active in your entries recently.`);
    }

    if (insights.length === 0) insights.push('Continue tracking to uncover deeper patterns between your mood and daily habits.');

    container.innerHTML = insights.map(i => `<p class="insight-text" style="margin-bottom:8px;">• ${i}</p>`).join('');
  }

  function renderRecommendations(mood) {
    const container = document.getElementById('recommendationsList');
    if (!mood) {
      container.innerHTML = '<p class="insight-text">Select a mood to see personalized tips.</p>';
      return;
    }

    let recs = [];
    const BASE_URL = '/MindHeaven/public/ug';
    const m = mood.toLowerCase();

    if (['stressed', 'anxious', 'angry'].includes(m)) {
      recs.push({ icon: '🧘', label: 'Guided Meditation', sub: 'Calm your mind in the Resource Hub', url: `${BASE_URL}/resources` });
      recs.push({ icon: '🤝', label: 'Counselor Meeting', sub: 'Talk it out with a professional', url: `${BASE_URL}/appointments` });
      recs.push({ icon: '📝', label: 'Stress Assessment', sub: 'Take a quick self-test', url: `${BASE_URL}/quiz` });
    } else if (['tired', 'sad'].includes(m)) {
      recs.push({ icon: '🛌', label: 'Sleep Guide', sub: 'Improve your sleep hygiene', url: `${BASE_URL}/resources` });
      recs.push({ icon: '🚶', label: 'Gentle Walk', sub: 'Log local exercise habit', url: `${BASE_URL}/habits` });
      recs.push({ icon: '🎵', label: 'Mood Boosters', sub: 'Self-care tips in resources', url: `${BASE_URL}/resources` });
    } else if (['happy', 'grateful', 'excited', 'calm'].includes(m)) {
      recs.push({ icon: '📓', label: 'Gratitude Habit', sub: 'Log a new positive habit', url: `${BASE_URL}/habits` });
      recs.push({ icon: '🌟', label: 'Self-Reflection', sub: 'Reflect on your wins in resources', url: `${BASE_URL}/resources` });
    } else {
      recs.push({ icon: '🧠', label: 'Mental Wellness', sub: 'Browse our Resource Hub', url: `${BASE_URL}/resources` });
      recs.push({ icon: '📊', label: 'Self Check-up', sub: 'Take a monthly assessment', url: `${BASE_URL}/quiz` });
    }
    container.innerHTML = recs.map(r => `
      <a href="${r.url}" class="rec-item">
        <div class="rec-icon">${r.icon}</div>
        <div class="rec-info">
          <span class="rec-label">${r.label}</span>
          <span class="rec-sub">${r.sub}</span>
        </div>
        <i class="fas fa-chevron-right" style="font-size:.7rem;color:var(--text-secondary);"></i>
      </a>
    `).join('');
  }

  // (renderReport removed)
