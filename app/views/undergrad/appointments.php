					...
					<form id="appointmentForm" class="appointment-form" novalidate>
						...
						<div class="form-grid">
							<div class="form-group">
								...
								<input type="text" id="appointmentTitle" name="title" class="input"
									placeholder="e.g., Dr. Smith - Therapy Session" required />
						...
						<div class="form-group">
							...
							<textarea id="appointmentNotes" name="notes" class="input" rows="3"
			...
			<div class="card-body">
				<div id="appointmentsEmptyState" class="empty-state" style="display:none;">
					<div class="empty-state-content">
						...
						<button class="btn" onclick="document.getElementById('appointmentTitle').focus()">Get
							Started</button>
					</div>
				</div>
...
<script>
	// Set BASE_URL before all other scripts
...
<script>
	...
	function loadCounselors() {
		const url = (window.BASE_URL || '/MindHeaven/public') + '/api/counselors';
		console.log('Loading counselors from:', url);
		fetch(url, { credentials: 'same-origin' })
			...
			.then(rows => {
				...
				if (!rows || !rows.length) {
					cSel.innerHTML = '<option value="">No counselors available</option>';
					return;
				}
				cSel.innerHTML = '<option value="">Select counselor</option>' +
					rows.map(c => `<option value="${c.id}">${c.full_name || c.username}</option>`).join('');
				console.log('Counselors loaded:', rows.length);
			})
			.catch(err => {
				console.error('Error loading counselors:', err.message);
				const cSel = document.getElementById('appointmentCounselor');
				cSel.innerHTML = '<option value="">Error loading counselors</option>';
			});
	}
